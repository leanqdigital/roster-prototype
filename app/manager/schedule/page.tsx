...
export default function ManagerSchedulePage() {
    ...
    return shifts
      ...
      .map((s) => {
        const assignment = shiftAssignments.find(
          (a) => a.shiftId === s.id && a.personId === myPerson.id,
        );
  ...
  }, [shifts, myAssignmentShiftIds, shiftAssignments, myPerson, viewStartStr, viewEndStr]);
  ...
  return (
    <div>
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-ink">My Schedule</h1>
          <p className="mt-0.5 text-xs text-ink-subtle">Your assigned shifts</p>
        </div>
      </div>
      ...
      {myShifts.length === 0 && (
        <div className="mt-8 rounded-xl border border-hairline bg-surface-2 p-10 text-center">
          ...
          <h2 className="mt-3 text-[15px] font-semibold text-ink">
            {view === "day" ? "No shifts this day" : view === "week" ? "No shifts this week" : "No shifts this month"}
          </h2>
          <p className="mx-auto mt-1 max-w-sm text-xs text-ink-muted">
            You don&apos;t have any shifts assigned for {view === "day" ? "this day" : view === "week" ? "this week" : "this month"}. Check back later or contact a company admin.
          </p>
        </div>
      )}
    </div>
