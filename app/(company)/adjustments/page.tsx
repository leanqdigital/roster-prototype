...
import type { ShiftAdjustmentRequest, ShiftAdjustmentStatus } from "@/lib/company-data";
...
const STATUS_FILTERS: { value: ShiftAdjustmentStatus | "all"; label: string }[] = [
  { value: "all", label: "All" },
...
export default function CompanyAdjustmentsPage() {
  const {
  ...
  const [statusFilter, setStatusFilter] = useState<ShiftAdjustmentStatus | "all">("all");
  ...
  const handleApprove = async (r: ShiftAdjustmentRequest) => {
    const result = await approveShiftAdjustment(r.id, reviewerName);
    pushToast(
      result.ok
        ? { tone: "success", message: "Adjustment approved" }
  ...
  const handleDeny = async () => {
    ...
    const result = await denyShiftAdjustment(denyTarget.id, reviewerName, denyComment);
    setDenyTarget(null);
    setDenyComment("");
    pushToast(
      result.ok
        ? { tone: "success", message: "Adjustment denied" }
        : { tone: "danger", message: result.error ?? "Couldn't deny — try again." },
    );
  };

  const handleUndo = async (id: string) => {
    const result = await revertShiftAdjustmentApproval(id, reviewerName);
    pushToast(
      result.ok
  ...
  }, [shiftAdjustmentRequests, statusFilter, teamFilter, personById]);
  ...
  const pendingCount = shiftAdjustmentRequests.filter((r) => r.status === "pending").length;
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="mt-8 rounded-xl border border-hairline bg-surface-2 p-10 text-center">
          <ClockIcon className="mx-auto size-10 text-ink-faint" />
